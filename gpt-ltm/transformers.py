...
from shared import load_vectors
...
# Returns a list of tuples (text_chunk, embedding) and an average embedding for a text.
...
# Get embeddings for a text using the OpenAI engine.
def get_embeddings(text_array, model):
    return openai.Embedding.create(input=text_array, model=model)['data']
...
# Get a unique ID for a file chunk
def get_unique_id_for_file_chunk(filename, chunk_index):
    return str(filename + "-!" + str(chunk_index))


# Creates text embeddings, and stores them in a Redis database for efficient searching and retrieval.
def handle_file_string(file, tokenizer, redis_conn, text_embedding_field, index_name):
    ...
    # Store the embedding vectors in Redis
    try:
